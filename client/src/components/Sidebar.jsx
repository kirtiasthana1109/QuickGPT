...
//   return (
//     <div className={`flex flex-col h-screen overflow-y-auto min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
...
//             )
//             .map((chat, index) => (  // index define kiya
//               <div onClick={() => { navigate('/'); setSelectedChat(chat); setIsMenuOpen(false) }}
//                 key={chat.id || index} // fallback ke liye index use kiya
...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  const {
    ...
    axios,
    setChats,
    fetchUsersChats,   // ✅ yaha sahi naam (pehle fetchUserChats tha)
    setToken,
    token
  } = useAppContext()
  ...
  const deleteChat = async (e, chatId) => {
    try {
      ...
      const { data } = await axios.post(
        ...
        { chatId },
        {
          headers: { Authorization: `Bearer ${token}` }  // ✅ main fix
        }
      )
        ...
        {chats
          ...
          .map((chat, index) => (
            <div
              ...
              key={chat._id || index}   // ✅ _id use kiya
              className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'
            >
              <div>
                <p className='truncate w-full'>
